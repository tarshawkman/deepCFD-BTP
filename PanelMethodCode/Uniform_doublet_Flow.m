...
lambda = 20;                                                                 % Source/sink strength
...
for i = 1:1:numX                                                            % Loop over X points
    for j = 1:1:numY                                                        % Loop over Y points
        ...
        Vx(i,j) = Vinf*cosd(alpha) -((lambda*(dx^2-dy^2))/(2*pi*((dx^2+dy^2)^2)));              % Compute X velocity component
        Vy(i,j) = Vinf*sind(alpha) -((lambda*dx*dy)/(pi*((dx^2+dy^2)^2)));              % Compute Y velocity component
    end
end
...
numT = 100000;                                                                 % Number of points along ellipse
