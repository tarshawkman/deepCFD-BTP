...
if __name__ == "__main__":
    ...
    # Apply Normalization
    norm_x = torch.max(torch.abs(x.reshape(x.shape[0], x.shape[1], -1)), dim=2)[0].max(dim=0)[0].view(1, -1, 1, 1)
    norm_y = torch.max(torch.abs(y.reshape(y.shape[0], y.shape[1], -1)), dim=2)[0].max(dim=0)[0].view(1, -1, 1, 1)
    ...
    import pickle
    import os
    pickle.dump(norm_x, open(os.path.join(simulation_directory, "norm_x.pkl"), "wb"))
    pickle.dump(norm_y, open(os.path.join(simulation_directory, "norm_y.pkl"), "wb"))
    ...
    DeepCFD, train_metrics, train_loss, test_metrics, test_loss = train_model(model, loss_func, train_dataset, test_dataset, optimizer,
        ...
        m_mse_on_batch=lambda scope: torch.sum((scope["output"].detach() - scope["batch"][1].detach()) ** 2).item(),
        m_mse_on_epoch=lambda scope: sum(scope["list"]) / len(scope["dataset"]),
        m_ux_name="Ux MSE",
        m_ux_on_batch=lambda scope: torch.sum((scope["output"][:,0,:,:].detach() - scope["batch"][1][:,0,:,:].detach()) ** 2).item(),
        m_ux_on_epoch=lambda scope: sum(scope["list"]) / len(scope["dataset"]),
        m_uy_name="Uy MSE",
        m_uy_on_batch=lambda scope: torch.sum((scope["output"][:,1,:,:].detach() - scope["batch"][1][:,1,:,:].detach()) ** 2).item(),
        m_uy_on_epoch=lambda scope: sum(scope["list"]) / len(scope["dataset"]),
        m_p_name="p MSE",
        m_p_on_batch=lambda scope: torch.sum((scope["output"][:,2,:,:].detach() - scope["batch"][1][:,2,:,:].detach()) ** 2).item(),
    ...
    torch.save(DeepCFD.state_dict(), simulation_directory + "modelWeights.pth")
    print(f"Model successfully saved to {simulation_directory + 'modelWeights.pth'}")
